...
output "api_url" {
  description = "API base URL → API_BASE=<url>/api and FRONTEND_ORIGIN's allowed frontend origins stay separate."
  value       = "https://${var.api_subdomain}.${var.domain}"
}

output "storefront_url" {
  description = "Canonical storefront URL (self-hosted on VPS, Caddy → PM2)."
