...
Разовый перенос данных из старой HTML-беты выполнялся скриптом
`backend/src/importFromBeta.js`, который удалён из репозитория 2026-09-17
(ADR 001 §1b-audit annex — историческое поведение сохранено в ADR и истории
git: `git log --follow -- backend/src/importFromBeta.js`). Пользователей и
пароли перенос не производил — старые пароли захешированы способом, который
не годится для сервера. Существующие клиенты один раз пройдут
"Забыли пароль" на новом сайте.
...
Проект распространяется под лицензией MIT (полный текст в `LICENSE`).
Код открытый, но лицензия не накладывает обязательств по раскрытию исходного
кода сервиса, используемого по сети — это позволяет использовать движок в
коммерческих white-label решениях. Не добавляйте код под несовместимой
лицензией и никогда не коммитьте секреты, ключи, `.env` или содержимое
`uploads/`.
